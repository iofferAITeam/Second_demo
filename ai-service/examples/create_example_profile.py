    ...
    try:
        ...
        hanyu_profile = StudentDocument(
            ...
            basicInformation=BasicInformation(
                name=Name(firstName="Hanyu", lastName="Liu"),
                contactInformation=ContactInformation(
                    phoneNumber=PhoneNumber(countryCode="+86", number="158-7235-5355"),
            ...
            educationBackground=EducationBackground(
                ...
                currentInstitution=CurrentInstitution(
                    name="Mount Holyoke College", type="overseas"
                ),
                major="Mathematics",
                gpa=Gpa(average="3.8/4.0", lastYear="3.41"),
                minor=Minor(hasMinor=None, name="", gpa="", motivation=""),
            ...
            practicalExperience=[
                PracticalExperienceItem(
                    ...
                    period=Period(startDate="August 2021", endDate="Present"),
                    ...
                    achievements="",
                    references=References(name="", position="", contact=""),
                ),
                PracticalExperienceItem(
                    type="other",
                    organization="GRUNDFOS Pumps",
                    role="Global Management Trainee",
                    period=Period(startDate="August 2019", endDate="August 2021"),
                    location="Shanghai, China",
                ...
                PracticalExperienceItem(
                    ...
                    period=Period(startDate="May 2017", endDate="July 2017"),
                ...
                PracticalExperienceItem(
                    ...
                    period=Period(startDate="January 2016", endDate="December 2017"),
            ...
            studyAbroadPreparation=StudyAbroadPreparation(
                internationalDegree=InternationalDegree(
                    desiredInstitution="", desiredProgram="", campusPreferences=[]
                ),
                livingExpenses=LivingExpenses(budget="", hasFamilySupport=False),
                additionalNeeds="",
        ...
        if saved_profile:
            ...
            print("=" * 60)
            print(
                f"👤 Name: {saved_profile.basicInformation.name.firstName} {saved_profile.basicInformation.name.lastName}"
            )
            print(
                f"📧 Email: {saved_profile.basicInformation.contactInformation.email}"
            )
            print(
                f"📱 Phone: {saved_profile.basicInformation.contactInformation.phoneNumber.countryCode} {saved_profile.basicInformation.contactInformation.phoneNumber.number}"
            )
            ...
            print(
                f"   • Intended Major: {saved_profile.applicationDetails.intendedMajor}"
            )
            print(
                f"   • Target Country: {saved_profile.applicationDetails.targetCountry}"
            )
            print(
                f"   • Application Year: {saved_profile.applicationDetails.applicationYear}"
            )
            print(
                f"   • Application Term: {saved_profile.applicationDetails.applicationTerm}"
            )
            print()
            print(f"📚 Education:")
            print(
                f"   • Current Institution: {saved_profile.educationBackground.currentInstitution.name}"
            )
            print(f"   • Major: {saved_profile.educationBackground.major}")
            print(f"   • GPA: {saved_profile.educationBackground.gpa.average}")
            print(
                f"   • Courses Completed: {len(saved_profile.educationBackground.coursesTaken)} courses"
            )
            print()
            ...
            if saved_profile.languageProficiency:
                ...
                print(
                    f"   • Reading: {test.scores.reading}, Listening: {test.scores.listening}"
                )
                print(
                    f"   • Speaking: {test.scores.speaking}, Writing: {test.scores.writing}"
                )
            print()
            print(
                f"💼 Work Experience: {len(saved_profile.practicalExperience)} positions"
            )
            for i, exp in enumerate(saved_profile.practicalExperience, 1):
                print(f"   {i}. {exp.role} at {exp.organization}")
                print(
                    f"      📍 {exp.location} ({exp.period.startDate} - {exp.period.endDate})"
                )
            print()
