...
class RAGDataPreparer:
    ...
    async def generate_all_embeddings(self, qa_pairs: List[Dict]) -> np.ndarray:
        ...
        print(
            f"📊 Processing {len(qa_pairs)} questions in batches of {self.batch_size}"
        )
    ...
    def build_faiss_index(self, embeddings: np.ndarray) -> faiss.Index:
        ...
        try:
            ...
            print(
                f"✅ FAISS index built: {index.ntotal} vectors, {dimension} dimensions"
            )
            return index
    ...
    def save_all_files(
        self, qa_pairs: List[Dict], embeddings: np.ndarray, index: faiss.Index
    ):
    ...
    def validate_setup(self, qa_pairs: List[Dict]) -> bool:
        ...
        try:
            ...
            assert loaded_embeddings.shape[0] == len(
                qa_pairs
    ...
    def print_summary(self, qa_pairs: List[Dict]):
        ...
        print(f"🗃️ Files available in data/ directory:")
