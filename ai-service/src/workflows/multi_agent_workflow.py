...
class ProfileConverter:
    ...
    def convert(user_profile: dict) -> Optional[StudentTagInfo]:
        ...
        try:
            ...
            # 语言成绩转换
            ProfileConverter._convert_language(
                user_profile, student_tags, LanguageLevel
            )
            ...
            # 研究经历转换
            ProfileConverter._convert_research(
                user_profile, student_tags, ResearchLevel
            )
            ...
            # 兴趣领域转换
            ProfileConverter._convert_interest(
                user_profile, student_tags, InterestField
            )

            # 默认设置
            student_tags.recommendation_level = RecommendationLevel.get_options()[
                "none"
            ]
            ...
            for lang_test in profile.get("languageProficiency", []):
                ...
                if "toefl" in test_type:
                    tags.language_level = (
                        options["high"]
                        if total_score >= 100
                        else options["medium"] if total_score >= 80 else options["low"]
                    )
                    ...
                    tags.language_level = (
                        options["high"]
                        if total_score >= 7.0
                        else options["medium"] if total_score >= 6.5 else options["low"]
                    )
            ...
            for test in profile.get("standardizedTests", []):
                if "gre" in test["type"].lower():
                    ...
                    tags.gre_level = (
                        options["high"]
                        if total_score >= 320
                        else options["medium"] if total_score >= 310 else options["low"]
                    )
    ...
    def _convert_research(profile: dict, tags: StudentTagInfo, ResearchLevel):
        ...
        try:
            ...
            research_exp = profile.get("practicalExperience", {}).get(
                "researchExperience", []
            )
    ...
    def _convert_papers(profile: dict, tags: StudentTagInfo, PaperLevel):
        ...
        try:
            ...
            count = sum(
                ...
                for exp in profile.get("practicalExperience", {}).get(
                    "researchExperience", []
                )
            )
            tags.paper_level = (
                options["many"]
                if count >= 3
                else options["weak"] if count >= 1 else options["none"]
            )
    ...
    def _convert_college(profile: dict, tags: StudentTagInfo, CollegeLevel):
        ...
        try:
            ...
            if any(kw in institution for kw in ["university", "college", "institute"]):
                ...
                kw in institution for kw in ["清华", "北大", "复旦", "交大", "中科大"]
            ):
...
class GeminiAPIClient:
    ...
    async def call(
        self, system_message: str, user_content: str, tools: Optional[List[Dict]] = None
    ) -> str:
        """
        统一的Gemini API调用方法
        ...
        try:
            ...
            response = await asyncio.wait_for(
                self.client.create(messages), timeout=self.timeout
            )
...
class MultiAgentSchoolRecommendationWorkflow:
    ...
    async def run_complete_recommendation(
        self, user_message: str, user_id: str = "default"
    ) -> str:
        """运行完整的推荐流程"""
        try:
            ...
            # 初始化状态对象
            state = WorkflowState(user_message=user_message, user_id=user_id)
...
"""
    ...
    async def _agent_3_final_recommendation(
...
async def run_multi_agent_recommendation(
