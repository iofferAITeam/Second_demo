...
def check_addmission_requirement(university: str, program: str):
    ...
    prompt = [
        {"role": "user", "content": f"What are the admission requirements for {program} at {university}?"}
    ]

    return perplexity_search(prompt)


def check_program_in_university(university: str, program: str, student_addmission_info: str, degree_type: str):
    """
    ...
    prompt = [
        {"role": "user", "content": f"{content} {student_addmission_info}"}
    ]
...
def get_qs_ranking(query: QSSubjectQuery):
    ...
    stmt = select(QSRanking).where(QSRanking.subject == query.subject, QSRanking.rank_2025_start <= query.rank, QSRanking.rank_2025_end >= query.rank)
...
def extract_test_scores_from_profile(profile):
    ...
    # Extract GPA
    if hasattr(profile.educationBackground, 'gpa') and hasattr(profile.educationBackground.gpa, 'average'):
        try:
    ...
    if hasattr(profile, 'standardizedTests') and profile.standardizedTests:
        for test in profile.standardizedTests:
            ...
            if 'gre' in test_type and hasattr(test.scores, 'total'):
                try:
                    scores["gre"] = float(test.scores.total) if test.scores.total else np.nan
                except (ValueError, AttributeError):
                    pass
            elif 'gmat' in test_type and hasattr(test.scores, 'total'):
                try:
                    scores["gmat"] = float(test.scores.total) if test.scores.total else np.nan
                except (ValueError, AttributeError):
                    pass
    ...
    if hasattr(profile, 'languageProficiency') and profile.languageProficiency:
        for lang_test in profile.languageProficiency:
            ...
            if 'ielts' in test_type and hasattr(lang_test.test.scores, 'total'):
                try:
                    scores["ielts"] = float(lang_test.test.scores.total) if lang_test.test.scores.total else np.nan
                except (ValueError, AttributeError):
                    pass
            elif 'toefl' in test_type and hasattr(lang_test.test.scores, 'total'):
                try:
                    scores["toefl"] = float(lang_test.test.scores.total) if lang_test.test.scores.total else np.nan
                except (ValueError, AttributeError):
                    pass
...
def get_prediction(student_info: StudentTagInfo):
    ...
    if profile.applicationDetails.targetCountry in qs_data["Country / Territory"].values:
        qs_data = qs_data[qs_data["Country / Territory"] == profile.applicationDetails.targetCountry]
    else:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            sys.executable,
            str(Path(__file__).resolve().parents[1] / "ml_models" / "predict_worker.py"),
            "--pipeline", str(Path("src/ml_models/xgboost_pipeline.joblib").resolve()),
            "--features-csv", str(features_path),
        ]
        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
        except Exception as e:
            raise RuntimeError(f"Predict subprocess failed to start: {type(e).__name__}: {e}")
        if proc.returncode != 0:
            stderr = (proc.stderr or "").strip()
            raise RuntimeError(f"Predict subprocess exited with {proc.returncode}: {stderr}")
        try:
            payload = json.loads(proc.stdout)
            probabilities = np.array(payload.get("probabilities", []), dtype=float)
        except Exception as e:
            raise RuntimeError(f"Failed to parse predict output: {type(e).__name__}: {e}")
...
def get_prediction_tool():
    return FunctionTool(get_prediction, description="Get the prediction for the user's profile")

def get_user_work_experience_tool():
    return FunctionTool(get_user_work_experience, description="Get the user's work experience")

def get_user_target_country_tool():
    return FunctionTool(get_user_target_country, description="Get the user's target country")

def get_user_application_details_tool():
    return FunctionTool(get_user_application_details, description="Get the user's program interest")

def get_complete_user_profile_tool():
    return FunctionTool(get_complete_user_profile, name="get_complete_user_profile", description="Get the complete user profile as JSON format")

def get_user_basic_information_tool():
    return FunctionTool(get_user_basic_information, description="Get the user's basic information (name, contact, nationality)")

def get_user_education_background_tool():
    return FunctionTool(get_user_education_background, description="Get the user's education background and academic history")

def get_user_language_proficiency_tool():
    return FunctionTool(get_user_language_proficiency, description="Get the user's language proficiency test scores")

def get_preplexity_tool():
    return FunctionTool(check_addmission_requirement, description="Get the admission requirement for a program in a university")

def get_qs_ranking_tool():
    return FunctionTool(get_qs_ranking, description="Get the QS ranking for a program in a university")
...
def demonstrate_text_to_numerical_conversion():
    ...
    print(f"   Papers: '{sample_student.paper_level.level}' → {tag_values['paper_tag']}")
    print(f"   Research: '{sample_student.research_level.level}' → {tag_values['research_tag']}")
    print(f"   College: '{sample_student.college_level.level}' → {tag_values['college_type_tag']}")
    print(f"   Interest Field: '{sample_student.interest_field.field_name}' → '{tag_values['interest_field']}'")
...
def create_sample_student_tag_info() -> StudentTagInfo:
    ...
    research_level = ResearchLevel(
        level="rich",
        description="rich research experience",
        prediction_value=0.7
    )
    
    college_level = CollegeLevel(
        level="overseas",
        description="overseas college",
    ...
    language_level = LanguageLevel(
        level="high",
        description="high language proficiency",
        prediction_value=0.9
    )
    
    paper_level = PaperLevel(
        level="none",
        description="no research papers",
...
def get_program_in_university_tool():
    return FunctionTool(check_program_in_university, description="Check if the program is offered in the university that the student can apply to")
